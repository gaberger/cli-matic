...
(deftest generate-global-help-test
  ...
  (is
    (= ["NAME:"
        ...
        ""
        "COMMANDS:"
        " add     [Adds two numbers together  Looks great, doesn't it?]"
...
(deftest multilines-global-help-test
  (is
    (= ["NAME:"
        ...
        ""
        "COMMANDS:"
        " mycmd     [A command description  Can contain multiple lines. Only the first line is displayed on global help.]"
        ""
        "GLOBAL OPTIONS:"
...
(deftest test-cl-format
  (let [input {:command     "subc",
               :description "Subtracts parameter B from A",
               :examples    "Just one example",
               :version     "1.2.3",
               :opts        [{:as "Parameter q", :default 0, :option "q", :type :int}],
               :subcommands
                            [{:command     "this-is-a-long command",
                              :description "This is a long command",
                              :opts
                                           [{:as      "Parameter A to subtract from",
                                             :default 0,
                                             :option  "a",
                                             :type    :int}
                                            {:as "Parameter B", :default 0, :option "b", :type :int}],
                              :runs        dummy-cmd}
                             {:command     "this-is-another-long-command",
                              :description "This is a another long command description",
                              :opts
                                           [{:as      "Parameter A to subtract from",
                                             :default 0,
                                             :option  "a",
                                            ...
                                            {:as "Parameter B", :default 0, :option "b", :type :int}],
                              :runs        dummy-cmd}
                             {:command     "this-is-another-yet-command",
                              :description "This is a another long command description",
                              :opts
                                           [{:as      "Parameter A to subtract from",
                                             :default 0,
    ...
    (is (= ["this-is-a-long command           This is a long command"
            "this-is-another-long-command     This is a another long command description"
            "this-is-another-yet-command      This is a another long command description"]
           #_(generate-global-command-list (:subcommands input))
           (let [padding (last (sort (mapv (fn [x]
                                             (count (:command x))) (:subcommands input))))
                 rows (mapv #(select-keys % [:command :description]) (:subcommands input))
...
(deftest test-cl-format-2
  (let [input {:command     "subc",
    ...
    (is (= ["this-is-a-long command           This is a long command"
            "this-is-another-long-command     This is a another long command description"
            "this-is-another-yet-command      This is a another long command description"]
           (generate-global-command-list (:subcommands input))
           ))))


(deftest test-cl-format-3
  (= ["NAME:"
      " toycalc - A command-line toy calculator"
      ""
      "USAGE:"
